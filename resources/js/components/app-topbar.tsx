...
export function AppTopbar({ breadcrumbs = [] }: { breadcrumbs?: BreadcrumbItem[] }) {
    ...
    const NAV_MODULES = useMemo(() => {
        ...
        return [...modules]
            ...
            .map((mod) => ({
                ...mod,
                items: [...mod.items].sort((a, b) => a.title.localeCompare(b.title)),
            }));
    }, [auth.user]);
                        ...
                        {NAV_MODULES.map((mod) => (
                            <div key={mod.key} className="mb-3">
                                <button
                                    ...
                                    onClick={() =>
                                        setOpenMobileModule(
                                            openMobileModule === String(mod.key) ? null : String(mod.key)
                                        )
                                    }
                                >
                                    {mod.module}
                                    <span>{openMobileModule === String(mod.key) ? '−' : '+'}</span>
                                </button>
                                        ...
                                        {mod.items.map((sub) => (
                                            <Link
                                                ...
                                                onClick={() => {
                                                    ...
                                                    localStorage.setItem('activeSubmodule', sub.id);
                                                    setMobileOpen(false);
                                                }}
                                            >
                                                {sub.title}
