...
const obtenerFechaOperacion = (
    ...
    const fechaFormateada = parseStringToDate(
...
const agruparOperaciones = (
    registros: Operacion[]
): FilaOperacion[] => {
    const operacionesOrdenadas = [...registros].sort(
        (a, b) => {
            const fechaA =
                obtenerFechaOperacion(a)?.getTime() ?? 0;
            const fechaB =
                obtenerFechaOperacion(b)?.getTime() ?? 0;
    ...
    const llegadasPendientes = new Map<
        string,
        FilaOperacion[]
    >();
    ...
    return filas.sort((a, b) => {
        const fechaA =
            obtenerFechaOperacion(a.llegada ?? a.salida)
                ?.getTime() ?? 0;

        const fechaB =
            obtenerFechaOperacion(b.llegada ?? b.salida)
                ?.getTime() ?? 0;
...
const formatearEstancia = (
    ...
    const diferencia = fechaSalida.getTime() -
...
export const exportarOperacionesAExcel = async (
    ...
    fechaCell.value =
        `Fecha de reporte: ${fechaGeneracion}`;
    fechaCell.font = {
        size: 10,
        color: { argb: 'FF475569' },
    };
    ...
    filas.forEach((fila, index) => {
        ...
        const fechaLlegada =
            obtenerFechaOperacion(llegada);

        const fechaSalida =
            obtenerFechaOperacion(salida);

        const fechaCsaeEntrada =
            obtenerFechaCsaeEntrada(fila);

        const fechaCsaeSalida =
            obtenerFechaCsaeSalida(fila);
        ...
        const values = [
            ...
            salida
                ? valorNumero(salida.pax)
                : '',
            salida
                ? valorNumero(salida.equipaje)
                : '',
            llegada?.tipo_cliente ??
                salida?.tipo_cliente ??
                '',
            formatearEstancia(
                fechaLlegada,
    ...
    worksheet.pageSetup.printArea =
        `A1:U${Math.max(6, filas.length + 5)}`;
    ...
    saveAs(
        blob,
        `Reporte_Operaciones_${fechaHoy}.xlsx`
    );
};
